/**
 * Enhanced Slash Commands for Conversation Tree Management
 * 
 * This module provides enhanced slash command functionality that integrates
...
/**
 * Enhanced chat command processor that adds tree functionality
 */
export class EnhancedSlashCommands {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.info('Enhanced slash commands initialized');
    } catch (error) {
      logger.error('Failed to initialize enhanced slash commands:', error);
      throw error;
    }
  }

  /**
   * Process enhanced chat commands with tree functionality
   */
  async processChatCommand(
    ...
    try {
      switch (subCommand) {
        ...
        default:
          return false; // Command not handled by enhanced system
      }
    } catch (error) {
      addItem({
        type: MessageType.ERROR,
        text: `Enhanced chat command error: ${error}`,
      });
      return true;
    }
  }
  ...
  private async handleBranchCommand(
    ...
    try {
      ...
      let tree = this.treeManager.getActiveTree();
      if (!tree) {
        tree = await this.treeManager.createTree('default', {
      ...
      const newNode = await this.treeManager.createBranch(tree.activeNodeId, branchName, {
        branchName,
  ...
  private async handleSwitchCommand(
    ...
    try {
      ...
      // Switch to the branch
              const switchedNode = await this.treeManager.switchToBranch(targetNode.id);
  ...
  private async handleMergeCommand(
    ...
    try {
      ...
      // Perform merge
      const mergeResult = await this.treeManager.mergeBranch(sourceNode.id, targetNode.id, strategy);

      if (mergeResult.success) {
        addItem({
          type: MessageType.INFO,
          text: `✅ Merge completed successfully!\n• Result node: ${mergeResult.resultNodeId}\n• Messages merged: ${mergeResult.metadata.mergedMessages}\n• Conflicts: ${mergeResult.metadata.conflictCount}\n• Resolution time: ${mergeResult.metadata.resolutionTime}ms\n• Preserved branches: ${mergeResult.metadata.preservedBranches.join(', ')}\n• Use "/chat tree" to see updated structure`,
        });
      } else {
        const conflictInfo = mergeResult.conflicts?.map(conflict => 
  ...
  private async handleTreeCommand(
    ...
    try {
      ...
      switch (action) {
        ...
        case 'analyze':
          const analysis = await this.treeManager.analyzeTree(tree.id);
          addItem({
            type: MessageType.INFO,
            text: `📊 Tree Analysis:\n• Complexity score: ${analysis.complexityScore}/10\n• Optimization potential: ${analysis.optimizationPotential}%\n• Recommended actions: ${analysis.recommendations.join(', ')}\n• Performance metrics: ${JSON.stringify(analysis.performance, null, 2)}`,
          });
          break;

        case 'optimize':
          const optimization = await this.treeManager.optimizeTree(tree.id, {
            compressOldNodes: true,
            mergeSimilarBranches: true,
            pruneEmptyBranches: true,
            defragmentStorage: true
          });
          addItem({
            type: MessageType.INFO,
            text: `⚡ Tree Optimization Complete:\n• Nodes compressed: ${optimization.nodesCompressed}\n• Branches merged: ${optimization.branchesMerged}\n• Storage saved: ${optimization.storageSaved}%\n• Performance improvement: ${optimization.performanceGain}%`,
          });
          break;
  ...
  private async handleNavigateCommand(
    ...
    try {
      ...
      const node = await this.treeManager.navigateToNode(nodeId);
      addItem({
        type: MessageType.INFO,
  ...
  private async handleHistoryCommand(
    ...
    try {
      if (showTree) {
        ...
        const history = await this.treeManager.getNodeHistory(tree.activeNodeId);
        const historyText = history.map((node, index) => 
  ...
  /**
   * Get help text for enhanced commands
   */
  getHelpText(): string {
    return `🌳 Enhanced Chat Commands (Tree Mode):
...
export const enhancedSlashCommands = new EnhancedSlashCommands(); 