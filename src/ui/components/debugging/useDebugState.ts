 ...
 */
const createMockDebugState = (): DebugState => {
  ...
  const watches: DebugWatchExpression[] = [
    {
      id: 'watch-1',
...
/**
 * Debug state hook return interface
 */
interface UseDebugStateResult {
  state: DebugState;
  commands: DebugCommand[];
  executeCommand: (command: string, args?: any[]) => void;
  updateVariable: (variable: DebugVariable, newValue: any) => void;
  toggleVariableExpansion: (variable: DebugVariable) => void;
  addBreakpoint: (path: string, line: number, condition?: string | null) => DebugBreakpoint;
  removeBreakpoint: (id: string) => void;
  updateBreakpoint: (id: string, updates: Partial<DebugBreakpoint>) => DebugBreakpoint;
  addWatch: (expression: string) => DebugWatchExpression;
  removeWatch: (id: string) => void;
  updateWatch: (id: string, updates: Partial<DebugWatchExpression>) => DebugWatchExpression;
  clearConsole: () => void;
 ...
 * 
 * @param initialState - Initial debug state
 * @returns Debug state and operations
 */
export const useDebugState = (initialState?: Partial<DebugState>): UseDebugStateResult => {
  ...
  const commands = useMemo<DebugCommand[]>(() => ([
    {
      name: 'continue',
    ...
    switch (command) {
      ...
      case 'step-into':
        setState(prev => {
          ...
          const newFrame: DebugStackFrame = {
            id: `frame-${Date.now()}`,
            name: 'processItem',
  ...
  // Update variable
  const updateVariable = useCallback((variable: DebugVariable, newValue: any) => {
    setState(prev => {
      ...
      const updateVariableRecursive = (vars: DebugVariable[], path: string): boolean => {
  ...
  const toggleVariableExpansion = useCallback((variable: DebugVariable) => {
    setState(prev => {
      // Clone the variables array
      const updatedVariables = JSON.parse(JSON.stringify(prev.variables));
      
      // Find and toggle the variable
      const toggleVariableRecursive = (vars: DebugVariable[], path: string): boolean => {
        for (let i = 0; i < vars.length; i++) {
          if (vars[i].path === path) {
  ...
  // Add breakpoint
  const addBreakpoint = useCallback((path: string, line: number, condition: string | null = null): DebugBreakpoint => {
  ...
  // Update breakpoint
  const updateBreakpoint = useCallback((id: string, updates: Partial<DebugBreakpoint>): DebugBreakpoint => {
    let updatedBreakpoint: DebugBreakpoint = {
      id: '',
      path: '',
      line: 0,
      enabled: true,
  ...
  // Add watch
  const addWatch = useCallback((expression: string): DebugWatchExpression => {
    let newWatch: DebugWatchExpression = {
      id: `watch-${Date.now()}`,
      expression,
  ...
  // Update watch
  const updateWatch = useCallback((id: string, updates: Partial<DebugWatchExpression>): DebugWatchExpression => {
    let updatedWatch: DebugWatchExpression = {
      id: '',
      expression: '',
      value: '',
      type: DebugVariableType.UNDEFINED,
  ...
  // Select stack frame
  const selectStackFrame = useCallback((frameId: string) => {
    setState(prev => {
