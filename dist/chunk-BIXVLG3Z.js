...
async function fileFromPath2(path3, ...args) {
  const { fileFromPath: _fileFromPath } = await import("./fileFromPath-ZQ3NZVPB.js");
...
var APIClient = class {
  ...
  methodRequest(method, path3, opts) {
    return this.request(Promise.resolve(opts).then((opts2) => ({ method, path: path3, ...opts2 })));
  }
...
async function executeWebFetch(input) {
  try {
    ...
    const fetchPromises = limitedUrls.map((url) => fetchUrlContent(url, extract_text_only));
...
async function fetchUrlContent(url, extractTextOnly) {
  try {
    const fetch3 = (await import("./src-ZDMU7XIS.js")).default;
...
function extractTextFromHtml(html) {
  ...
  if (text.length > 8e3) {
    text = text.substring(0, 8e3) + "... [content truncated]";
  }
...
function createCodeAnalyzerTool() {
  return {
    name: "analyze_code",
...
function analyzeSecurity(content, language) {
  const security = {
    score: 100,
    vulnerabilities: [],
    warnings: []
...
function generateSuggestions(analysisResults, content, language) {
  const suggestions = [];
...
**Issues:**
`;
    results.quality.issues.forEach((issue) => output += `- ${issue}
`);
  }
  output += `
`;
  output += `## \u{1F512} Security Analysis (Score: ${results.security.score}/100)
`;
  if (results.security.vulnerabilities.length > 0) {
    output += `
**\u{1F6A8} Vulnerabilities:**
`;
    results.security.vulnerabilities.forEach((vuln) => output += `- ${vuln}
`);
  }
...
**\u26A0\uFE0F Warnings:**
`;
    results.security.warnings.forEach((warning) => output += `- ${warning}
`);
  }
  output += `
`;
  if (results.suggestions && results.suggestions.length > 0) {
    output += `## \u{1F4A1} Recommendations
`;
...
function registerBuiltInTools() {
  ...
  toolRegistry.register({
    ...
    try {
      const fs2 = await import("fs/promises");
      await fs2.writeFile(input.path, input.content, "utf-8");
      return { success: true };
    } catch (error) {
      return {
        success: false,
  ...
  toolRegistry.register({
    ...
    try {
      const fs2 = await import("fs/promises");
      const files = await fs2.readdir(input.path);
...
var Claude4Client = class {
  ...
  constructor(apiKey, config) {
    ...
    this.client = new sdk_default({
      ...
      timeout: config?.api?.timeout || 6e4
    });
   ...
   */
  setModel(model) {
    if (Object.values(CLAUDE_4_MODELS).includes(model)) {
...
function createClaude4Client(apiKey, config) {
...
var appConfigSchema = external_exports.object({
  ...
  api: external_exports.object({
    baseUrl: external_exports.string().url().default("https://api.anthropic.com"),
    version: external_exports.string().default("v1"),
    timeout: external_exports.number().positive().default(6e4),
    key: external_exports.string().optional()
  }).default({}),
  // AI configuration
  ai: external_exports.object({
    model: external_exports.string().default(CLAUDE_4_MODELS.CLAUDE_4_SONNET),
    temperature: external_exports.number().min(0).max(1).default(0.5),
    maxTokens: external_exports.number().int().positive().default(4096),
    maxHistoryLength: external_exports.number().int().positive().default(20),
    enableCaching: external_exports.boolean().default(true),
    enableTools: external_exports.boolean().default(true),
    enableTelemetry: external_exports.boolean().default(true),
    enableBetaFeatures: external_exports.boolean().default(true),
    autoModelSelection: external_exports.boolean().default(true),
    costBudget: external_exports.number().positive().default(100),
    performanceMode: external_exports.enum(["speed", "balanced", "quality"]).default("balanced"),
    systemPrompt: external_exports.string().default(
      "You are Claude, a helpful AI assistant. You are running in Claude Code, a CLI tool for software development.You help users with programming tasks, code generation, debugging, and other software development activities.Respond in a concise, direct manner appropriate for a command-line interface."
    )
  }).default({}),
  // Authentication configuration
  auth: external_exports.object({
    autoRefresh: external_exports.boolean().default(true),
    tokenRefreshThreshold: external_exports.number().positive().default(300),
    maxRetryAttempts: external_exports.number().int().positive().default(3)
  }).default({}),
  // Terminal configuration
  terminal: external_exports.object({
    theme: external_exports.enum(["dark", "light", "system"]).default("system"),
    useColors: external_exports.boolean().default(true),
    showProgressIndicators: external_exports.boolean().default(true),
    codeHighlighting: external_exports.boolean().default(true),
    useHighContrast: external_exports.boolean().default(false),
    fontSizeAdjustment: external_exports.enum(["small", "normal", "large"]).default("normal"),
    reduceMotion: external_exports.boolean().default(false),
    simplifyInterface: external_exports.boolean().default(false),
    streamingSpeed: external_exports.number().positive().default(40)
  }).default({}),
  // Accessibility configuration
  accessibility: external_exports.object({
    enabled: external_exports.boolean().default(false),
    disableLoadingPhrases: external_exports.boolean().default(false),
    screenReaderOptimized: external_exports.boolean().default(false),
    keyboardNavigationEnhanced: external_exports.boolean().default(false)
  }).default({}),
  // Telemetry configuration
  telemetry: external_exports.object({
    enabled: external_exports.boolean().default(true),
    submissionInterval: external_exports.number().positive().default(30 * 60 * 1e3),
    maxQueueSize: external_exports.number().int().positive().default(100),
    autoSubmit: external_exports.boolean().default(true)
  }).default({}),
  // File operation configuration
  fileOps: external_exports.object({
    maxReadSizeBytes: external_exports.number().positive().default(10 * 1024 * 1024)
  }).default({}),
  // Execution configuration
  execution: external_exports.object({
    shell: external_exports.string().default("bash")
  }).default({}),
  // Logger configuration
  logger: external_exports.object({
    level: external_exports.string().default("info"),
    timestamps: external_exports.boolean().default(true),
    colors: external_exports.boolean().default(true)
  }).default({}),
  // Claude 4 specific configuration
  claude4: external_exports.object({
    vision: external_exports.boolean().default(true),
    visionEnhancements: external_exports.object({
      detail: external_exports.enum(["low", "high"]).default("high")
    }).default({}),
    preferredModel: external_exports.enum([
    ...
    fallbackModel: external_exports.string().default(CLAUDE_4_MODELS.CLAUDE_4_HAIKU)
  }).default({}),
  // Version
  version: external_exports.string().default("0.3.0")
});
...
var CONFIG_PATHS = [
  // Current directory
  path2.join(process.cwd(), ".claude-code.json"),
  path2.join(process.cwd(), ".claude-code.js"),
  // User home directory
  path2.join(os.homedir(), ".claude-code", "config.json"),
  path2.join(os.homedir(), ".claude-code.json"),
  // XDG config directory (Linux/macOS)
  process.env.XDG_CONFIG_HOME ? path2.join(process.env.XDG_CONFIG_HOME, "claude-code", "config.json") : path2.join(os.homedir(), ".config", "claude-code", "config.json"),
  // AppData directory (Windows)
  process.env.APPDATA ? path2.join(process.env.APPDATA, "claude-code", "config.json") : ""
].filter(Boolean);
var ConfigManager = class {
  config;
  schema;
  options;
  constructor(initialConfig, schema, options = {}) {
    this.config = initialConfig;
    this.schema = schema;
    this.options = {
      configPath: path2.join(os.homedir(), ".claude-code", "config.json"),
      envPrefix: "CLAUDE_",
      validate: true,
      loadFromFile: true,
      loadFromEnv: true,
  ...
  mergeConfigs(base, overlay) {
    ...
    for (const key in overlay) {
      if (Object.prototype.hasOwnProperty.call(overlay, key)) {
        ...
        if (typeof baseValue === "object" && baseValue !== null && !Array.isArray(baseValue) && typeof overlayValue === "object" && overlayValue !== null && !Array.isArray(overlayValue)) {
          result[key] = this.mergeConfigs(baseValue, overlayValue);
        } else {
   ...
   */
  async save(configPath = this.options.configPath) {
    try {
...
async function saveConfig(config, configPath = path2.join(os.homedir(), ".claude-code", "config.json")) {
  try {
    await fs.mkdir(path2.dirname(configPath), { recursive: true });
...
async function loadConfig(options = {}) {
  const manager = new ConfigManager(defaults, appConfigSchema, options);
  await manager.initialize();
  return manager.get();
}
var config_default = new ConfigManager(defaults, appConfigSchema);
...
humanize-ms/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-BIXVLG3Z.js.map