...
var ConversationHistoryManager = class {
  ...
  async listSessions() {
    try {
      ...
      return sessions.filter((s) => s !== null).sort((a, b) => b.startTime - a.startTime);
    } catch (error) {
  ...
  async cleanupOldSessions() {
    try {
      const sessions = await this.listSessions();
      const now = Date.now();
      const maxAge = this.options.maxAgeInDays * 24 * 60 * 60 * 1e3;
      const oldSessions = sessions.filter(
        (s) => now - s.startTime > maxAge
      );
      for (const session of oldSessions) {
        await this.deleteSession(session.id);
      }
      const recentSessions = sessions.filter((s) => now - s.startTime <= maxAge).sort((a, b) => b.startTime - a.startTime);
...
var conversationHistory = new ConversationHistoryManager();
export {
  ConversationHistoryManager,
  conversationHistory
};
//# sourceMappingURL=conversation-history-XB6JGOEN.js.map