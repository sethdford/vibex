...
describe('Error Analyzer', () => {
  ...
  describe('getResolutionSteps function', () => {
    ...
    test('should provide specific resolution steps for authentication errors', () => {
      const steps = getResolutionSteps(ErrorCategory.AUTHENTICATION);
      expect(steps.some(step => step.includes('API key'))).toBe(true);
    });
    
    test('should provide specific resolution steps for network errors', () => {
      const steps = getResolutionSteps(ErrorCategory.NETWORK);
      expect(steps.some(step => step.includes('internet connection'))).toBe(true);
    });
    
    test('should provide specific resolution steps for rate limit errors', () => {
      const steps = getResolutionSteps(ErrorCategory.RATE_LIMIT);
